...
class DoctorSpecializationService {
    ...
    async create(doctor_id: string, names: string[]){
        ...
        if(!doctorNotExist){
            throw new ServiceError(400, "Doctor already exist.", "ERROR DoctorSpecializationService > create > doctorNotExist");
        }   
        ...
        for (const name of names) {
            ...
            if(!specialization){            
                throw new ServiceError(400, "Specialization not exist.", "ERROR DoctorSpecializationService > create > specialization not exist");
            }
